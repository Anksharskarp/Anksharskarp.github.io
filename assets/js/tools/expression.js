...
export function compileExpression(
  ...
  if (source.length > 256)
    throw new Error("Keep each equation under 257 characters.");
  const tokens = [];
  const pattern =
    /\s*(?:(\d*\.\d+|\d+\.?\d*)([eE][+-]?\d+)?|([a-zA-Z]+)|([+\-*/^()]))/y;
  let position = 0;
  while (position < source.trimEnd().length) {
    ...
    if (!match)
      throw new Error(
        `Unexpected character at position ${position + 1}. Use * for multiplication.`,
      );
    tokens.push(
      match[1]
  ...
  if (tokens.length > 128)
    throw new Error("This equation has too many terms. Simplify it first.");
  let cursor = 0;
  const program = [];
  ...
  if (cursor !== tokens.length)
    throw new Error(
      "Check parentheses and use * for multiplication, for example 2*y.",
    );
  ...
  return (values) => {
    ...
    for (const [operation, argument] of program) {
      if (operation === "number") stack[top++] = argument;
        ...
        stack[top++] = Object.hasOwn(values, argument) ? values[argument] : NaN;
      else if (operation === "function")
        stack[top - 1] = argument(stack[top - 1]);
      else {
      ...
      if (!Number.isFinite(stack[top - 1]))
        throw new Error(
          "The equation is undefined or outside the real number range at this point.",
        );
    }
    return stack[0];
  };
